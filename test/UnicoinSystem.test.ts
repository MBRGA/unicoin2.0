...
contract("Unicoin Registry Full system test 🧪🔬", (accounts) => {
  ...
  before(async function () {
    ...
    licenceManager.initialize("Unicoin NFT Licence", "NFT", unicoinRegistry.address);
    ...
    await vault.initialize(daiContract.address, unicoinRegistry.address);

    await unicoinRegistry.initialize(
  ...
  context("User Management 💁‍♂️", function () {
    ...
    it("Revert if user already added", async () => {
      await expectRevert.unspecified(
        unicoinRegistry.registerUser(exampleUserProfileURI, {
          from: publisher,
        })
      );
    });
  ...
  context("Publication Management: Reject on negative input 🙅‍♂️‍", function () {
    it("Reverts if invalid publication: input non-registered user", async () => {
      await expectRevert.unspecified(
        unicoinRegistry.createPublication(
          samplePublication_FixedRate._pricing_Strategy,
          samplePublication_FixedRate._publication_uri,
          samplePublication_FixedRate._auctionFloor,
          samplePublication_FixedRate._auctionStartTime,
  ...
  context("Publication Management: retrieve publication information 🔎", function () {
    ...
    it("Correctly get publication information", async () => {
      ...
      assert.equal(
        fixedRatePublication[0].toNumber(),
        samplePublication_FixedRate._pricing_Strategy,
        "Incorrectly set _pricing_Strategy for fixed sale"
      );
      assert.equal(
        fixedRatePublication[1],
        samplePublication_FixedRate._publication_uri,
        "Incorrectly set _publication_uri for fixed sale"
      );
      assert.equal(fixedRatePublication[2].toNumber(), 1, "Incorrectly set author_id for fixed sale");
      assert.equal(
        fixedRatePublication[3].toNumber(),
        samplePublication_FixedRate._fixed_sell_price,
        "Incorrectly set sell_price for fixed sale"
      );
      assert.equal(
        fixedRatePublication[4].toNumber(),
        samplePublication_FixedRate._maxNumberOfLicences,
        "Incorrectly set maxNumberOfLicences for fixed sale"
      );
      ...
      assert.equal(
        fixedRatePublication[7][0],
        samplePublication_FixedRate._contributors[0],
        "Incorrectly set _contributors for fixed sale"
      );
      assert.equal(
        fixedRatePublication[7][1],
        samplePublication_FixedRate._contributors[1],
        "Incorrectly set _contributors for fixed sale"
      );
      assert.equal(
        fixedRatePublication[8][0].toNumber(),
        samplePublication_FixedRate._contributors_weightings[0],
        "Incorrectly set _contributors_weightings for fixed sale"
      );
      assert.equal(
        fixedRatePublication[8][1].toNumber(),
        samplePublication_FixedRate._contributors_weightings[1],
        "Incorrectly set _contributors_weightings for fixed sale"
      );

      let auctionPublication = await unicoinRegistry.getPublication.call(1);
      assert.equal(
        auctionPublication[0].toNumber(),
        samplePublication_PrivateAuction._pricing_Strategy,
        "Incorrectly set _pricing_Strategy for auction"
      );
      assert.equal(
        auctionPublication[1],
        samplePublication_PrivateAuction._publication_uri,
        "Incorrectly set _publication_uri for auction"
      );
      assert.equal(auctionPublication[2].toNumber(), 1, "Incorrectly set author_id for auction");
      assert.equal(
        auctionPublication[3].toNumber(),
        samplePublication_PrivateAuction._fixed_sell_price,
        "Incorrectly set sell_price for auction"
      );
      assert.equal(
        auctionPublication[4].toNumber(),
        samplePublication_PrivateAuction._maxNumberOfLicences,
        "Incorrectly set maxNumberOfLicences for auction"
      );
      ...
      assert.equal(
        auctionPublication[7][0],
        samplePublication_PrivateAuction._contributors[0],
        "Incorrectly set _contributors for auction"
      );
      assert.equal(
        auctionPublication[7][1],
        samplePublication_PrivateAuction._contributors[1],
        "Incorrectly set _contributors for auction"
      );
      assert.equal(
        auctionPublication[8][0].toNumber(),
        samplePublication_PrivateAuction._contributors_weightings[0],
        "Incorrectly set _contributors_weightings for auction"
      );
      assert.equal(
        auctionPublication[8][1].toNumber(),
        samplePublication_PrivateAuction._contributors_weightings[1],
        "Incorrectly set _contributors_weightings for auction"
      );
    });
  });
  ...
  context("Auction Bidding: Reveal stage 💵", function () {
    ...
    it("Revert if wrong user tries to reveal bid", async () => {
      ...
      await expectRevert.unspecified(
        unicoinRegistry.revealSealedBid(sealedBid2.bidAmount, sealedBid2.salt, 1, 0, {
          from: bidder2,
        })
      );
    });
    ...
    it("Revert if wrong time to reveal bid", async () => {
      ...
      await expectRevert.unspecified(
        unicoinRegistry.revealSealedBid(sealedBid1.bidAmount, sealedBid1.salt, 1, 0, {
          from: bidder1,
        })
      );
    });
  ...
  context("Auction Bidding: finalize auction 🏁", function () {
    it("Can finalize auction correctly", async () => {
      ...
      let expectedPublisherBalanceAfter =
        sealedBid2.bidAmount *
        (1 -
          (samplePublication_PrivateAuction._contributors_weightings[0] +
            samplePublication_PrivateAuction._contributors_weightings[1]) /
            100);
      assert.equal(
        publisherBalanceAfter.toNumber(),
        expectedPublisherBalanceAfter,
        "Publisher balance not correctly set"
      );

      let contributor1BalanceAfter = await daiContract.balanceOf(contributor1);
      let expectedContributor1BalanceAfter =
        (sealedBid2.bidAmount * samplePublication_PrivateAuction._contributors_weightings[0]) / 100;
      assert.equal(
        contributor1BalanceAfter.toNumber(),
        expectedContributor1BalanceAfter,
        "Contributor1 balance after wrong"
      );

      let contributor2BalanceAfter = await daiContract.balanceOf(contributor2);
      let expectedContributor2BalanceAfter =
        (sealedBid2.bidAmount * samplePublication_PrivateAuction._contributors_weightings[1]) / 100;
      assert.equal(
        contributor2BalanceAfter.toNumber(),
        expectedContributor2BalanceAfter,
        "Contributor2 balance after wrong"
      );

      let bidder2BalanceAfter = await daiContract.balanceOf(bidder2);
      let expectedBidder2BalanceAfter =
        bidder2BalanceBefore -
        (expectedPublisherBalanceAfter + expectedContributor1BalanceAfter + expectedContributor2BalanceAfter);
      assert.equal(
        bidder2BalanceAfter.toNumber(),
        expectedBidder2BalanceAfter,
        "Bidder2 balance after not set correctly"
      );
  ...
  context("Harberger Tax: publication management 📕", function () {
    ...
    it("Can bid (and win) auction on harberger tax", async () => {
      ...
      await unicoinRegistry.commitSealedBid(
        sealedBid2.bidHash,
        2, //2 is the auction number for the just created publication
        {
          from: bidder2,
        }
      );
    ...
    it("Can get info about tax object", async () => {
      ...
      Object.keys(taxObject).forEach(function (key) {
        if (key != 2) {
          ...
          assert.equal(
            taxObject[key].toNumber() <= contractTime,
            true,
  ...
  context("Harberger Tax: claiming claimHarbergerTax 👮‍♂️", function () {
    ...
    it("correctly updates tax object after paying tax", async () => {
      ...
      assert.equal(
        contractTime.toNumber() - taxObject[2].toNumber() < 5,
        true,
        "did not correctly update last payment date"
      );
    });
  });
  context("Harberger Tax: buyout creation 💳", function () {
    ...
    it("Can correctly create a valid buyout request", async () => {
      await unicoinRegistry.createHarbergerBuyOut(1, sealedBid2.bidAmount * 1.1, {
        from: bidder1,
      });
      ...
      let expectedObject = {
        0: 0, //taxObject_Id
        1: 2, //buyOutOwner_Id
        2: sealedBid2.bidAmount * 1.1, //buyOutAmount
      ...
      Object.keys(buyout).forEach(function (key) {
        if (key != 3) {
          ...
          assert.equal(
            buyout[key] - expectedObject[key] < 5,
            true,
            "The delta between expected and actual time should be < 5"
          );
        }
      });
    });
    ...
    it("Reverts if not a harberger tax licence", async () => {
      await expectRevert.unspecified(
        unicoinRegistry.createHarbergerBuyOut(0, sealedBid2.bidAmount * 1.1, {
          from: bidder1,
        })
      );
    });
    it("Reverts if value sent less than min buyout price", async () => {
      await expectRevert.unspecified(
        unicoinRegistry.createHarbergerBuyOut(1, sealedBid2.bidAmount * 1.04, {
          from: bidder1,
        })
      );
    });
  ...
  context("Harberger Tax: buyout finilization 🏎", function () {
    ...
    it("correctly updates tax object information", async () => {
      ...
      assert.equal(taxObject[4], sealedBid2.bidAmount * 1.1, "Should have the correct outbid amount");
    });
  ...
  context("Harberger Tax: solvent licence owner 🔏", function () {
    it("places licence for auction if owner is underfunded", async () => {
      ...
      assert.equal(newAuctionInformation[3].toNumber(), 60 * 60 * 24 * 30, "auction duration error");
      assert.equal(newAuctionInformation[4][0], undefined, "there should be no bids associated with the auction");
      assert.equal(newAuctionInformation[5], 0, "currently no winning bid");
      assert.equal(newAuctionInformation[6], 0, "status should be commit");
    });
    it("can bid on auction and win new licence", async () => {
      ...
      let publicationLicences = await unicoinRegistry.getPublicationLicences(2);
      assert.equal(
        publicationLicences[1].toNumber(),
        2,
        "Licence number 2 should be added to the array of licences for the object"
      );
