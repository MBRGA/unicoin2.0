    ...
    <v-card class="mx-auto pa-7">
      <v-row>
        <v-col>
          ...
          <v-card-subtitle>
            <div>
              <span class="text-subtitle-1 pr-4"
                >{{ publicationInformation.authorFirstName }} {{ publicationInformation.authorLastName }}</span
              >
              <img
                alt="ORCID logo"
              ...
              />
              <a :href="`https://orcid.org/${publicationInformation.authorOrcid}`" target="_blank" class="text-body-2"
                >https://orcid.org/{{ publicationInformation.authorOrcid }}</a
              >
            </div>
          ...
          <v-card-text class="pb-7">
            ...
            <div v-for="bid in publicationInformation.bids" :key="bid.id">
              <div class="text-body-2" v-if="bid.status === 0">
                <div v-if="publicationInformation.isAuction">
                  ...
                  <p>
                    <span class="font-weight-bold">Offered by: </span>&nbsp; {{ bid.bidderFirstName }}
                    {{ bid.bidderLastName }}, {{ bid.bidderCompanyName }}
                  </p>
                  <v-row class="py-4 justify-end">
                    <v-btn color="primary" class="ma-4 px-10" @click="showReject"> Reject </v-btn>
                    <v-btn color="primary" class="ma-4 px-10" @click="showAccept"> Accept </v-btn>
                    <v-dialog v-model="acceptDialog.visible" persisent max-width="290">
                      <v-card>
                        ...
                        <v-card-actions>
                          ...
                          <v-btn text @click="processAccept('', false)"> Cancel </v-btn>
                        </v-card-actions>
                      </v-card>
                    </v-dialog>
                  </v-row>
            ...
            <v-row class="text-caption" no-gutters>
              <v-col class="font-weight-bold" cols="4" align-self="start">Commercial licensing details: </v-col>
              <v-col v-if="publicationInformation.isAuction" align-self="start"
                >You have elected to sell commercial licences to your work through an auction.
              </v-col>
              <v-col v-else align-self="start">
                <v-row no-gutters> You have elected to sell commercial licences at a flat rate. </v-row>
                <v-row no-gutters>
                  <span class="font-weight-bold">Flat rate: </span>&nbsp;
                  {{ formatter.format(publicationInformation.sellPrice) }}
                </v-row>
              </v-col>
            </v-row>
            <v-spacer></v-spacer>
            <v-row class="justify-end py-4">
              <v-col cols="8">
                <v-chip
                  class="my-4 py-1"
                  color="green"
                  :outlined="publicationInformation.isRunning"
                  :disabled="publicationInformation.isRunning"
                  @click="publicationInformation.isRunning = !publicationInformation.isRunning"
                >
              ...
              <v-spacer></v-spacer>
              <v-col cols="2">
                <v-btn class="ma-4 px-10" color="accent" @click="showChangePrice"> Change </v-btn>
              </v-col>
            </v-row>
            <v-dialog v-model="priceDialog.visible" persistent max-width="500">
              <v-card>
                ...
                <v-card-text class="mt-5">
                  <v-row class="justify-space-around">
                    <v-col
                      cols="5"
                      align-self="center"
                      class="text-right body-1 font-weight-bold"
                      :class="{ 'text--disabled': priceDialog.isAuction }"
                      >Fixed Price</v-col
                    >
                    <v-col>
                    ...
                    <v-col
                      cols="5"
                      align-self="center"
                      class="text-left body-1 font-weight-bold"
                      :class="{ 'text--disabled': !priceDialog.isAuction }"
                      >Auction</v-col
                    >
                  </v-row>
                ...
                <v-card-text>
                  <Transition name="switch-text" mode="out-in">
                    <div v-if="!priceDialog.isAuction">
                      <p>
                        Your publication will be licensed at a fixed price. Please select the price at which you would
                        like to license your work:
                      </p>
                    </div>
                    <p v-else>
                      Your publication will now be for sale through an auction. Bidders will submit prices to you, which
                      you will need to review and accept or reject.
                    </p>
                  </Transition>
                  <v-expand-transition>
                    <v-text-field
                      v-model="priceDialog.newPrice"
                      v-if="!priceDialog.isAuction"
                      @beforeinput="limitDigits"
                      label="Flat rate"
                      prefix="USD $"
                      :rules="[rules.price]"
                      type="number"
                ...
                <v-card-actions>
                  ...
                  <v-btn
                    @click="priceDialog.confirmationDialog = true"
                    text
                    :disabled="
                      priceDialog.newPrice === publicationInformation.sellPrice &&
                      priceDialog.isAuction === publicationInformation.isAuction
                    "
                ...
                <v-dialog v-model="priceDialog.confirmationDialog" persistent max-width="350">
                  <v-card>
                    ...
                    <v-card-text class="mt-5">
                      ...
                      <p v-else>
                        Are you sure you wish to sell rights to this publication for a fixed price of $
                        {{ formatter.format(priceDialog.newPrice) }}?
                      </p>
                      <v-card-actions>
                        <v-spacer></v-spacer>
...
import { Publication, useStore } from "@/store/piniastore";
...
const acceptDialog: AcceptDialog = reactive({
  visible: false,
  title: "Accept Bid?",
...
const priceDialog: PriceDialog = reactive({
  visible: false,
  isAuction: true,
  newPrice: props.publicationInformation.sellPrice,
