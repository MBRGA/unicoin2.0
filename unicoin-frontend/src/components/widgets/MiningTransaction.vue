<template>
  <v-dialog v-if="miningTransactionObject.status != null">
    <v-card>
      
    </v-card>
    <v-dialog-header> Poop chute </v-dialog-header>
  </v-dialog>
  <div v-if="miningTransactionObject.status != null" class="text-center">
    <md-dialog
      class="text-center"
      v-model:md-active="miningTransactionObject.status != null"
    ...
    >
      ...
      />
      <p style="padding: 30px" v-if="miningTransactionObject.status == 'uploading'">
        Your content is being uploaded to IPFS. This could take a few seconds.
      </p>
      <p style="padding: 30px" v-if="miningTransactionObject.status == 'pending'">
        Approve the transaction in your web3 provider to submit it to the blockchain.
      </p>
      ...
      <md-button v-if="miningTransactionObject.status == 'done'" class="md-primary md-raised" @click="modalClosed"
        >Close</md-button
      >
    </md-dialog>
  </div>
