<template>
  <v-card class="mx-auto pa-7">
    <v-card-title>{{ publicationInformation.title }}</v-card-title>
    <v-card-subtitle>
      <div>
        <span class="text-subtitle-1 pr-4">{{ publicationInformation.authorFirstName }} {{ publicationInformation.authorLastName }}</span>
        <img alt="ORCID logo" style="height: 0.875rem" src="../assets/orcid.png" />
    ...
    <v-card-text>
      ...
      <div class="text-caption">{{ publicationInformation.abstract }}</div>
      <div class="text-subtitle-2">Keywords</div>
      <div class="text-caption">{{ publicationInformation.keyword.toString() }}</div>
    </v-card-text>
    <v-card-text>
    ...
    <div class="md-layout">
      <div class="md-layout-item">
        ...
        <p>
          <b>Published by: </b>
          <b>
            <i>{{ publicationInformation.authorFirstName }} {{ publicationInformation.authorLastName }}</i> </b
          >, {{ publicationInformation.authorUniversity }}.
          <a v-bind:href="'https://orcid.org/' + publicationInformation.authorOrcid" target="_blank">
        ...
        <div v-for="bid in publicationInformation.bids" :key="bid">
          <div v-if="bid.status == 0">
            ...
            <p v-if="publicationInformation.isAuction">
              <b>Offered by:</b> {{ bid.bidderFirstName }} {{ bid.bidderLastName }}{{ bid.bidderCompanyName }}
            </p>
            <!-- <p v-if="bid.bidderAccountType='company'"><b>Offered by:</b> {{bid.bidderCompanyName}}</p> -->
            <md-dialog v-model:md-active="showDialog1">
            ...
            <md-dialog v-model:md-active="showDialog2">
              <md-tabs md-dynamic-height>
                <md-tab md-label="Confirmation">
            ...
            <md-button v-if="publicationInformation.isAuction" @click="showDialog1 = true" class="md-primary md-raised"
              >Accept</md-button
            >
            <md-button v-if="publicationInformation.isAuction" @click="showDialog2 = true" class="md-raised md-accent"
              >Reject</md-button
            >
          </div>
        </div>
        ...
        <p v-if="publicationInformation.isAuction">
          <b>Commercial licencing details:</b> You have elected to sell commercial licences to your work through an
          auction.
        </p>
        <p v-if="!publicationInformation.isAuction">
        ...
        <md-button v-if="!publicationInformation.isAuction" @click="showDialog5 = true" class="md-primary md-raised"
          >CHANGE LICENCE PRICE</md-button
        >
        <md-button v-if="!publicationInformation.isAuction" @click="showDialog4 = true" class="md-primary md-raised"
          >CHANGE TO AUCTION</md-button
        >
        <md-button v-if="publicationInformation.isAuction" @click="showDialog3 = true" class="md-primary md-raised"
          >CHANGE TO SALE</md-button
        >

        <md-dialog v-model:md-active="showDialog3">
          <md-tabs md-dynamic-height>
        ...
        <md-dialog v-model:md-active="showDialog4">
          <md-tabs md-dynamic-height>
            <md-tab md-label="Change to auction">
              <p>
                Your publication will now be for sale through an auction. Bidders will submit prices to you, which you
                will need to review and accept or reject.
              </p>
            </md-tab>
          </md-tabs>
        ...
        <md-dialog v-model:md-active="showDialog5">
          <md-tabs md-dynamic-height>
        ...
        <p>
          <b>Running status: </b> <md-chip v-if="publicationInformation.isRunning" class="md-primary">Running</md-chip
          ><md-chip v-if="publicationInformation.isRunning" md-clickable @click="changeRunningStatus"
            >Stop running</md-chip
          >
          <md-chip v-if="!publicationInformation.isRunning" class="md-accent">Not running</md-chip
          ><md-chip v-if="!publicationInformation.isRunning" md-clickable @click="changeRunningStatus"
            >Start running</md-chip
          >
        </p>
      </div>
